...
- CLI: `prepare`, `lint`, `generate-index`, `query` (owned strategies only). No vector `build` — tree-reason is vectorless; eval sidecars are built by `evals.bench` / `evals.index`.
